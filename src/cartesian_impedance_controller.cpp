...
CallbackReturn CartesianImpedanceController::on_init() {
 ...
 /*   position_d_target_msg.x =  position_d_target_(0);
    position_d_target_msg.y =  position_d_target_(1);
   position_d_target_msg.z =  position_d_target_(2);
   std::cout << "position_d_target_msg looks like this" << position_d_target_msg.x << std::endl;
   //position_d_target_msg.yaw =  position_d_target(3);
   //position_d_target_msg.roll =  position_d_target(4);
   //position_d_target_msg.pitch =  position_d_target(5);
...
controller_interface::return_type CartesianImpedanceController::update(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) {  
  ...
  */
  ...
  case 1:
    Theta = Lambda;
    F_impedance = -1 * (D * (jacobian * dq_) + K * error /*+ I_error*/) + F_safety + F_hold;
    break;
